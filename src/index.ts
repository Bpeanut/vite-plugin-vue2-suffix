...
function VitePluginVue2Suffix() {
  return {
    ...
    transform(code: string, id: string) {
       ...
       * @param {String} compShortPath
       */
      const handleImport = (filePathRaw: string, compShortPath: string) => {
      ...
      const handleVueSuffixImport = () => {
        if (id.includes(".vue") && !id.includes("?")) {
          ...
          if (compoArr) {
            const { pathList = [], componentShortPath = [] } = TOOLS.handleGetImportUrl(
              compoArr,
              code,
              id
            );

            if (compoArr.length && pathList) {
      ...
      const addSuffixStaticImportRoute = () => {
        ...
        const pathList =
          newRouterImportArr && TOOLS.handleFullPath(newRouterImportArr, id);
